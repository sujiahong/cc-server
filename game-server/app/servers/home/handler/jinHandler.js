...
handle.applyRoomList = function(msg, session, next){
    var self = this;
    var servers = self.app.getServersByType("game_jin");
    var parallelFunc = [];
    for (let i = 0, len = servers.length; i < len; ++i){
        var func = (cb)=>{
            homeRPC.rpcGameListPublicRoom(constant.GAME_PLAY.zha_jin_hua, i+1, cb);
        }
...
handle.match = function(msg, session, next){
    ...
    util.genRoomUniqueId(redis.existRoomId, function(err, roomId){
        ...
        console.log(TAG, userId, "  11mammama  ", roomId)
        msg.roomId = roomId;
        var gamePlay = constant.GAME_PLAY.zha_jin_hua;

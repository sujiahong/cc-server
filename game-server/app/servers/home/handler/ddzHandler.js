...
module.exports = function(app){
    return new DDZHandler(app);
}

var DDZHandler = function(app){
    this.app = app;
}

var handle = DDZHandler.prototype;

handle.applyRoomList = function(msg, session, next){
    var self = this;
    var servers = self.app.getServersByType("game_ddz");
    var parallelFunc = [];
    for (let i = 0, len = servers.length; i < len; ++i){
        var func = (cb)=>{
            homeRPC.rpcGameListPublicRoom(constant.GAME_PLAY.dou_di_zhu, i+1, cb);
        }
...
handle.match = function(msg, session, next){
    ...
    if (session.get("roomId")){
        return next(null, {code: errcode.HAVE_IN_DDZ_ROOM});
    }
    util.genRoomUniqueId(redis.existRoomId, function(err, roomId){
        ...
        msg.roomId = roomId;
        var gamePlay = constant.GAME_PLAY.dou_di_zhu;

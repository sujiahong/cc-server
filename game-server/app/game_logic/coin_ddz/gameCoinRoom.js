"use_strict";
const TAG = "coinDDZRoom.js";
const constant = require("../../shared/constant");
const inspect = require("./inspectDDZ");
...
module.exports = CoinDDZRoom;

var temp = function(){};
temp.prototype = baseRoom.prototype;
CoinDDZRoom.prototype = new temp();

var room = CoinDDZRoom.prototype;

room.init = function (roomId, rule) {
    this.initBase(roomId, rule);
    var len = 55;
    var poker = constant.DDZ_CARD_POKER;
    if (this.rulePlay.budai345 == 1){
        len = 43;
        poker = constant.DDZ_CARD_WITHOUT345;
    }
    for (var k = 1; k < len; ++k){
...
room.dealCard = function(num) {
    ...
    for(i = 0; i < 3; ++i){
        this.auxiliaryCards.push(pokerCards[i]);
    }
    ...
    var handData = {};
    handData.landlordCards = this.auxiliaryCards;
    for (var uid in players){
        var player = players[uid];
        var arr = [];
...
room.robLandlord = function(userId, robScore){
    if (robScore == 0){
        var operationDataLen = this.operationDataArr.length;
        if (operationDataLen == 4){
            if (this.roomLaw == constant.DDZ_PLAY_TYPE.jiaofen){
                return this.lookupDealerId();
            }else{
                if (!this.isHavePlayerRob(-2)){
                    return this.lookupDealerId();
                }
                return errcode.OK;
            }
        }else if (operationDataLen > 4 && this.roomLaw == constant.DDZ_PLAY_TYPE.jiaodizhu){
            return this.lookupDealerId();
        }
        return errcode.OK;
    }else{
...
room.isHavePlayerRob = function(score){
    var players = this.players;
    for (var uid in players){
        if (players[uid].robScore == score){
            return true;
        }
    }
    return false;
...
room.lookupDealerId = function(){
    var dealerId = "";
    var operationDataArr = this.operationDataArr;
    var tail = operationDataArr.length-1;
    var score1 = 1, score2 = 2;
    if (this.roomLaw == constant.DDZ_PLAY_TYPE.jiaodizhu){
        score1 = -1;
        score2 = -2;
    }
    for (var i = tail; i > 0; --i){
        var robData = operationDataArr[i];
        if (robData.robScore == score2 || robData.robScore == score1){
            this.dealerId = robData.userId;
            return errcode.OK;
        }
    }
    if (dealerId) {
    ...
    if (player.trusteeStat == 0){
        g_ddzGameMgr.comeinTrustee(roomId, operateId, ()=>{
            if (room.dealerId){
                ...
                var robScore = ponderRob.getLevelOneThinkRobScore(room, operateId);
                g_ddzGameMgr.robLandlord(roomId, operateId, robScore, (err, data)=>{console.log("222222222", err, data)});
            }
        });
    }else{///已经托管了
        if (room.dealerId){
